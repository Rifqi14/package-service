...
func (c *Consumer) ReConnect(queueName, bindingKey, exchange string) (<-chan amqp.Delivery, error) {
	...
	deliveries, err := c.AnnounceQueue(queueName, bindingKey, exchange)
	if err != nil {
...
// connection...
func (c *Consumer) AnnounceQueue(queueName, bindingKey, exchange string) (<-chan amqp.Delivery, error) {
	args := amqp.Table{
...
func (c *Consumer) Handle(
	...
	for {
		...
		if <-c.done != nil {
			d, err = c.ReConnect(queue, routingKey, exchange)
			if err != nil {
